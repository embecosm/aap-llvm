...
public:
  AAPAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
  ...
  unsigned checkTargetMatchPredicate(MCInst &Inst) override {
    ...
    const MCRegisterClass &MRC =
        getParser().getContext().getRegisterInfo()->getRegClass(
            AAP::GR8RegClassID);
...
/// AAPOperand - Instances of this class represent a parsed machine instruction
...
public:
  ...
  static bool isImm10(const MCExpr *I) {
    int64_t Res;
    if (I->evaluateAsAbsolute(Res))
      return isUInt<10>(Res);
    return true;
  }
  static bool isImm12(const MCExpr *I) {
    int64_t Res;
    if (I->evaluateAsAbsolute(Res))
      return isUInt<12>(Res);
    return true;
  }
  ...
  static bool isField16(const MCExpr *I) {
    int64_t Res;
    if (I->evaluateAsAbsolute(Res))
      return isInt<16>(Res) || isUInt<16>(Res);
    return true;
  }
  ...
  static bool isShiftConst3(const MCExpr *I) {
    ...
    int64_t Res;
    I->evaluateAsAbsolute(Res);
    return isUInt<3>(Res - 1);
  }
  static bool isShiftImm6(const MCExpr *I) {
    int64_t Res;
    if (I->evaluateAsAbsolute(Res))
      return isUInt<6>(Res - 1);
    return true;
  }
...
std::unique_ptr<AAPOperand> AAPAsmParser::ParseMemSrc() {
  ...
  if (!RegOp) {
    Error(getLoc(), "Missing register in memsrc operand");
    return nullptr;
  }
  RegNo = RegOp->getReg();
  ...
  if (!getLexer().is(AsmToken::Comma)) {
    Error(getLoc(), "Missing ',' separator in memsrc operand");
    return nullptr;
  }
  getParser().Lex();
  ...
  if (!ImmOp) {
    Error(getLoc(), "Missing immediate in memsrc operand");
    return nullptr;
  }
