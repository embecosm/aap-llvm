...
uint32_t AAPSimState::getPC() { return pc_w; }
...
uint8_t AAPSimState::getCodeMem(uint32_t address) {
  ...
  if (debug_trace)
    llvm::dbgs() << " CODEMEM READ: 0x" << llvm::format("%07" PRIx64, address)
                 << ": 0x" << llvm::format("%02" PRIx64, code_memory[address])
                 << "\n";
...
void AAPSimState::setCodeMem(uint32_t address, uint8_t val) {
  ...
  if (debug_trace)
    llvm::dbgs() << " CODEMEM WRITE: 0x" << llvm::format("%07" PRIx64, address)
                 << ": 0x" << llvm::format("%02" PRIx64, val) << "\n";
...
uint8_t AAPSimState::getDataMem(uint32_t address) {
  ...
  if (debug_trace)
    llvm::dbgs() << " DATAMEM READ: 0x" << llvm::format("%04" PRIx64, address)
                 << ": 0x" << llvm::format("%02" PRIx64, code_memory[address])
                 << "\n";
...
void AAPSimState::setDataMem(uint32_t address, uint8_t val) {
  ...
  if (debug_trace)
    llvm::dbgs() << " DATAMEM WRITE: 0x" << llvm::format("%04" PRIx64, address)
                 << ": 0x" << llvm::format("%02" PRIx64, val) << "\n";
