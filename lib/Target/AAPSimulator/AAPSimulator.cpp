...
AAPSimulator::AAPSimulator() {
  ...
  IP = TheTarget->createMCInstPrinter(Triple("aap-none-none"),
...
SimStatus AAPSimulator::exec(MCInst &Inst, uint32_t pc_w, uint32_t &newpc_w) {
  ...
  case AAP::NOP_short: {
    ...
    switch (Command) {
    case 0:
      return SimStatus::SIM_BREAKPOINT;
  ...
  case AAP::ADD_r_short: {
    ...
    EXCEPT(uint32_t ValA = State.getReg(RegSrcA));
    EXCEPT(uint32_t ValB = State.getReg(RegSrcB));
    uint32_t Res = ValA + ValB;
    EXCEPT(State.setReg(RegDst, static_cast<uint16_t>(Res)));
    // Test for overflow
    State.setOverflow(static_cast<uint16_t>(Res) != Res ? 1 : 0);
    break;
  }
  ...
  case AAP::LDW_predec_short: {
    ...
    bool predec = (Inst.getOpcode() == AAP::LDB_predec ||
                   ...
                   Inst.getOpcode() == AAP::LDW_predec_short)
                      ? true
                      : false;
    bool word =
        (Inst.getOpcode() == AAP::LDW || Inst.getOpcode() == AAP::LDW_short ||
         Inst.getOpcode() == AAP::LDW_postinc ||
         Inst.getOpcode() == AAP::LDW_postinc_short ||
         Inst.getOpcode() == AAP::LDW_predec ||
         Inst.getOpcode() == AAP::LDW_predec_short)
  ...
  case AAP::STW_predec_short: {
    ...
    bool predec = (Inst.getOpcode() == AAP::STB_predec ||
                   ...
                   Inst.getOpcode() == AAP::STW_predec_short)
                      ? true
                      : false;
    bool word =
        (Inst.getOpcode() == AAP::STW || Inst.getOpcode() == AAP::STW_short ||
         Inst.getOpcode() == AAP::STW_postinc ||
         Inst.getOpcode() == AAP::STW_postinc_short ||
         Inst.getOpcode() == AAP::STW_predec ||
         Inst.getOpcode() == AAP::STW_predec_short)
  ...
  case AAP::BAL_short: {
    ...
    int16_t SImm = (Inst.getOpcode() == AAP::BAL)
  ...
  case AAP::BLEU_short: {
    uint16_t Imm = Inst.getOperand(0).getImm();
    EXCEPT(uint16_t ValA =
               State.getReg(getLLVMReg(Inst.getOperand(1).getReg())));
    int16_t SValA = static_cast<int16_t>(ValA);
    EXCEPT(uint16_t ValB =
               State.getReg(getLLVMReg(Inst.getOperand(2).getReg())));
    int16_t SValB = static_cast<int16_t>(ValB);
    bool longbr =
        (Inst.getOpcode() == AAP::BEQ_ || Inst.getOpcode() == AAP::BNE_ ||
         Inst.getOpcode() == AAP::BLTS_ || Inst.getOpcode() == AAP::BLES_ ||
         Inst.getOpcode() == AAP::BLTU_ || Inst.getOpcode() == AAP::BLEU_)
            ? true
            : false;
    int16_t SImm = longbr ? signExtendBranchCC(Imm) : signExtendBranchCCS(Imm);
    bool branch = false;
  ...
  case AAP::BRA_short: {
    ...
    int32_t SOffset = (Inst.getOpcode() == AAP::BRA)
                          ? signExtendBranch(Offset)
